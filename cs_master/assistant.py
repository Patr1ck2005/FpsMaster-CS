...
class Assistant:
    ...
    def reset_reaction(self):
        if self.shooting_mode == 'Spray':
            maxlen = self.REACTION_TICK * self.TARGET_CONTINUITY_SCALE
        else:
            if self.weapon_mode == 'AWP':
                maxlen = self.REACTION_TICK
            else:
                maxlen = self.REACTION_TICK * (self.TARGET_CONTINUITY_SCALE // 3)
        self.raw_pos_info_memory = [deque([None] * maxlen, maxlen=maxlen),
                                    deque([None] * maxlen, maxlen=maxlen)]
    ...
    def get_info(self, info):
        ...
        if self.shooting:  # auto_pressing ignore
            self.recoil_time = ((time() - self._start_shooting_time)
                                + min(.8, max(0, (
                            self.last_spray_last - 2.5 * (self._start_shooting_time - self._end_shooting_time)))))
        else:
            self.recoil_time = max(0, min(.8, max(0, (
                    self.last_spray_last - 2.5 * (time() - self._end_shooting_time)))) - 3 * (
        ...
        if info[0] is not None and not (
            ...
            if (not self.lost_target) and any(self.raw_pos_info_memory[0]) and self.fix_auto_shoot:
                ...
                self.x_cold_down = 0
                self.y_cold_down = 0
                if self.shooting_mode == 'Spray' and self.shooting and self.target_r_pos is not None:
                    self.filter_predict(self.delay_time)
                    self.process_relative_pos()
    ...
    def filter_predict(self, delay):
        ...
        self.pred_rebuild_pixel_pos[0], self.pred_rebuild_pixel_v[0], self.pred_rebuild_pixel_pos[1], \
            ...
            = motion_translation_matrix(delay + self.OPERATION_DELAY + 0.02) @ self.kf.x
