...
class Detector:
    ...
    def find_target(self, prediction):
        for det in prediction:
            ...
            for *_rect_xyxy, _confidence, _class in det:
                ...
                if _class == 3:
                    marker_offset = self.cal_offset(target_pos)
                    # target_iou = self.box_iou_xyxy(rect_xyxy, self.old_xyxy, 5)
                    if marker_offset < _offset:
                        _marker_offset = marker_offset
                        self.marker_pos_r = (mid_x, mid_y)
                if head_track and _class == 1:
    ...
    def get_target_pos_size(self):
        if (self.target_xx[0] is not None and self.marker_pos_r[0] is not None and
                self.target_xx[0] < self.marker_pos_r[0] < self.target_xx[1]):
    ...
    def get_assist_info(self, pred_pixel_r_pos, mouse_rebuild, pred_rebuild_pixel_pos):
        if pred_pixel_r_pos is not None:
            self.pred_pixel_r_pos = int(pred_pixel_r_pos[0]+160), int(pred_pixel_r_pos[1]+160)
        else:
            self.pred_pixel_r_pos = None
        self.mouse_rebuild = int(mouse_rebuild[0]+160), int(mouse_rebuild[1]+160)
        if pred_rebuild_pixel_pos is not None:
            self.pred_rebuild_pixel_pos = int(pred_rebuild_pixel_pos[0]+160), int(pred_rebuild_pixel_pos[1]+160)
        else:
            self.pred_rebuild_pixel_pos = None
