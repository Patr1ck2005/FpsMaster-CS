...
def download_model_checkpoint(opt, experiment):
    ...
    if len(model_asset_list) == 0:
        logger.detect_error(f'COMET ERROR: No checkpoints found for model name : {model_name}')
        return
    ...
    if asset_id is None:
        logger.detect_error(f'COMET ERROR: Checkpoint {checkpoint_filename} not found in the given Experiment')
        return
